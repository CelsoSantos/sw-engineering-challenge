...
const populateBloqs = (filepath: string) => {
  const bloqs = fs.readFileSync(filepath, { encoding: 'utf8' });
...
const populateLockers = (filepath: string) => {
  const lockers = fs.readFileSync(filepath, { encoding: 'utf8' });
  JSON.parse(lockers).forEach((locker: Locker) => {
    lockerCollection.push(
      new Locker(locker.id, locker.bloqId, locker.status, locker.isOccupied),
    );
  });
};

const populateRents = (filepath: string) => {
  const rents = fs.readFileSync(filepath, { encoding: 'utf8' });
  JSON.parse(rents).forEach((rent: Rent) => {
    rentCollection.push(
      new Rent(rent.id, rent.weight, rent.size, rent.status, rent.lockerId),
    );
  });
};
...
const listenChangesinArray = (arr: any, callback: any) => {
  ...
  ['pop', 'push', 'splice', 'sort'].forEach((m: any) => {
    arr[m] = (...args: any) => {
      const res = Array.prototype[m].apply(arr, args); // call normal behaviour
      ...
      if (m === 'splice' && arr[0] instanceof Bloq) {
        // Check first element type and assume the type from there
        callback.apply(arr, [bloqsFile, bloqCollection]); // finally call the callback supplied
      }
      if (m === 'splice' && arr[0] instanceof Locker) {
        // Check first element type and assume the type from there
        callback.apply(arr, [lockersFile, lockerCollection]); // finally call the callback supplied
      }
      if (m === 'splice' && arr[0] instanceof Rent) {
        // Check first element type and assume the type from there
        callback.apply(arr, [rentsFile, rentCollection]); // finally call the callback supplied
      }
      return res;
